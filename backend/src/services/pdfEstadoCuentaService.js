...
const estadoCuentaService = require("./estadoCuentaService");
  ...
  async generatePDFFile(idclienteprov, year, filePath) {
    ...
    return new Promise((resolve, reject) => {
      ...
      doc
        ...
        .fontSize(7.5)
        .font("Helvetica-Bold")
        .text("REPORTE - INGRESOS", marginLeft + contentWidth - 130, currentY)
        .font("Helvetica")
        .fontSize(7.5)
        .text(fechaHora, marginLeft + contentWidth - 130, currentY + 12);

      // Línea divisoria formal debajo del header
      ...
      const titulo = "CTAS. CORRIENTE - INGRESOS POR CLIENTE";
      doc.fillColor("black");
      doc.fontSize(15).font("Helvetica-Bold");
      const tituloWidth = doc.widthOfString(titulo) + 40;
      const tituloHeight = doc.currentLineHeight() + 16;
      const tituloX = marginLeft + (contentWidth - tituloWidth - 40) / 2;
      const tituloY = currentY;

      // Cuadro del título con doble borde (estilo formal)
      doc
        .lineWidth(0.8)
      ...
      // Cuadro para información del cliente (líneas sutiles)
      doc
        .lineWidth(0.5)
      ...
      const columns = [
        { name: "Item", x: colX, width: columnWidths.item },
        {
          ...
          width: columnWidths.recibo,
        },
        {
          name: "Fecha",
          x: (colX += columnWidths.recibo),
      ...
      // Header de la tabla con estilo más sutil
      doc.fillColor("black");
      ...
      ingresos.forEach((transaccion, index) => {
        ...
        doc.text(
          transaccion.tipo === "INGRESO" ? transaccion.id.toString() : "-",
          columns[1].x + cellPadding,
          cellY,
          { width: columns[1].width - cellPadding * 2, align: "center" }
        );
      ...
      // Footer con Total Anual (estilo formal)
      ...
      // Calcular total solo de ingresos
      const totalIngresos = ingresos.reduce(
        (sum, ing) => sum + (parseFloat(ing.importe) || 0),
        0
      );
      const totalAnualNumero = parseFloat(totalIngresos).toFixed(2);
      const totalAnualFormateado = formatearMoneda(totalAnualNumero.toString());
      doc.fillColor("black");
      doc
        .fontSize(11)
        .font("Helvetica-Bold")
        ...
        .fontSize(12)
        .text(totalAnualFormateado, totalBoxX + 10, totalY + 18, {
