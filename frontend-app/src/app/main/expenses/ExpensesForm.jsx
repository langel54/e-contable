...
import { createSalida, updateSalida } from "@/app/services/expensesServices";
...
const ExpensesForm = ({ salidaEdit = null, handleCloseModal }) => {
  ...
  const handleSubmit = async (values, setFieldError, setSubmitting) => {
    try {
      ...
      let response;
      if (salidaEdit) {
        response = await updateSalida(salidaEdit.idsalida, valoresProcesados);
      } else {
        response = await createSalida(valoresProcesados);
      }
      handleCloseModal();
    } catch (error) {
      const errorMessage =
            ...
            <Grid container spacing={2}>
              <Grid item xs={12} md={4}>
                <TextField
                  ...
                  name="fecha"
                  type="datetime-local"
                  value={dayjs(values.fecha).format("YYYY-MM-DDTHH:mm")}
              ...
              <Grid item xs={12}>
                ...
                >
                  {salidaEdit ? "Actualizar Egreso" : "Registrar Egreso"}
                </Button>
              </Grid>
            </Grid>
          </Form>
